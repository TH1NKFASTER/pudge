...
local tracking_file = os.getenv('ANIME_MPV_ANILIST_TRACKING_FILE') or ''
local python = os.getenv('ANIME_MPV_PYTHON') or 'python3'
local config_path = os.getenv('ANIME_MPV_CONFIG') or ''
local threshold = tonumber(os.getenv('ANIME_MPV_ANILIST_THRESHOLD') or '0.8333333333') or 0.8333333333
local max_remaining_minutes = tonumber(os.getenv('ANIME_MPV_ANILIST_MAX_REMAINING_MINUTES') or '10') or 10
max_remaining_minutes = math.max(0, max_remaining_minutes)
local media_id = os.getenv('ANIME_MPV_ANILIST_MEDIA_ID') or ''
local title = os.getenv('ANIME_MPV_ANILIST_TITLE') or 'anime'
local auto_update = (os.getenv('ANIME_MPV_ANILIST_AUTO_UPDATE') or '0') == '1'
local shortcut_mark_watched = os.getenv('ANIME_MPV_SHORTCUT_MARK_WATCHED') or 'Ctrl+a'
local shortcut_open_anilist = os.getenv('ANIME_MPV_SHORTCUT_OPEN_ANILIST') or 'Ctrl+b'
local shortcut_correct_match = os.getenv('ANIME_MPV_SHORTCUT_CORRECT_MATCH') or 'c'
local ui_language = (os.getenv('ANIME_MPV_UI_LANGUAGE') or 'en'):lower()
local app_name = os.getenv('ANIME_MPV_APP_NAME') or 'Anime MPV'
local app_cli = os.getenv('ANIME_MPV_APP_CLI') or 'anime-mpv'
...
local playback_enabled = (os.getenv('ANIME_MPV_PLAYBACK_ENABLED') or '0') == '1'
local playback_video = os.getenv('ANIME_MPV_PLAYBACK_VIDEO') or ''
local playback_interval = tonumber(os.getenv('ANIME_MPV_PLAYBACK_INTERVAL') or '30') or 30
...
local subtitle_refresh_enabled = (os.getenv('ANIME_MPV_SUBTITLE_REFRESH') or '1') ~= '0'
...
local function refresh_after_first_subtitle(reason)
    ...
    -- Anime MPV. Embedded and bitmap tracks must never participate.
...
if shortcut_mark_watched ~= '' then
    add_reliable_binding(shortcut_mark_watched, 'anime_mpv_anilist_update', function() update_anilist(true) end)
end
if shortcut_open_anilist ~= '' then
    add_reliable_binding(shortcut_open_anilist, 'anime_mpv_anilist_open', open_anilist)
end
if shortcut_correct_match ~= '' then
    mp.add_key_binding(shortcut_correct_match, 'anime_mpv_anilist_correct', correct_anilist)
end
