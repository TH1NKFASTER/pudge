...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path(os.environ.get("ANIME_MPV_CONFIG", str(DEFAULT_CONFIG_PATH))),
    )
...
def process_video(
    ...
    if args.fast_play and anime_hint is not None and tracking_available:
        ...
        tracking_episode = args.episode_hint or _anilist_episode(identity, anime_hint)
        title = anime_hint.titles[0] if anime_hint.titles else str(anime_hint.id)
        print(
    ...
    tracker_script: Path | None = Path(
        str(files("anime_mpv").joinpath("mpv_scripts/anime_mpv_anilist.lua"))
    )
    tracker_env: dict[str, str] = {
        "ANIME_MPV_PYTHON": sys.executable,
        "ANIME_MPV_CONFIG": str(config.config_path),
        "ANIME_MPV_UI_LANGUAGE": config.ui.language,
        "ANIME_MPV_APP_NAME": APP_NAME,
        "ANIME_MPV_APP_CLI": APP_CLI,
        "ANIME_MPV_PLAYBACK_ENABLED": "1" if config.playback.enabled else "0",
        "ANIME_MPV_PLAYBACK_VIDEO": str(video),
        "ANIME_MPV_PLAYBACK_INTERVAL": str(config.playback.save_interval_seconds),
        "ANIME_MPV_SHORTCUT_MARK_WATCHED": config.shortcuts.mpv_mark_watched,
        "ANIME_MPV_SHORTCUT_OPEN_ANILIST": config.shortcuts.mpv_open_anilist,
        "ANIME_MPV_SHORTCUT_CORRECT_MATCH": config.shortcuts.mpv_correct_match,
    }
    if tracking_anime is not None and tracking_episode is not None and config.anilist.access_token:
        ...
        tracker_env.update({
            "ANIME_MPV_ANILIST_TRACKING_FILE": str(tracking_file),
            "ANIME_MPV_ANILIST_THRESHOLD": str(threshold),
            "ANIME_MPV_ANILIST_MAX_REMAINING_MINUTES": str(max(0.0, config.anilist.watched_max_remaining_minutes)),
            "ANIME_MPV_ANILIST_MEDIA_ID": str(tracking_anime.id),
            "ANIME_MPV_ANILIST_TITLE": title,
            "ANIME_MPV_ANILIST_AUTO_UPDATE": "1" if tracking_auto else "0",
            "ANIME_MPV_PYTHON": sys.executable,
            "ANIME_MPV_CONFIG": str(config.config_path),
        })
...
def main(argv: list[str] | None = None) -> int:
    ...
    background_prepare = os.environ.get("ANIME_MPV_BACKGROUND_PREPARE") == "1"
