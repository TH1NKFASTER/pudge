...
_HELPER_FLAG = "--pudge-native-notification"
_HELPER_ACTIVE_ENV = "PUDGE_NOTIFICATION_HELPER_ACTIVE"
_HELPER_PATH_ENV = "PUDGE_NOTIFICATION_HELPER"
...
def send_native_notification_direct(subtitle: str, message: str, timeout: float = 8.0) -> bool:
    ...
    This function must run from the executable inside ``pudge.app`` so macOS
...
def send_native_notification(subtitle: str, message: str) -> bool:
    """Send a macOS notification from the installed pudge app bundle.

    Background work normally runs under Homebrew Python, whose notification
    identity differs from ``pudge.app``. It therefore invokes a hidden mode
