...
(() => {
  ...
  async function renderCurrent(text, lineId) {
    ...
    if(parsed.has(lineId)){root.innerHTML=window.PudgeReadingTools?.study?.renderParsedText(parsed.get(lineId),options)||esc(text);return;}
    root.textContent=text;
    try{const payload=await pywebview.api.visual_novel_parse(text);parsed.set(lineId,payload);if(lastLineId===lineId)root.innerHTML=window.PudgeReadingTools?.study?.renderParsedText(payload,options)||esc(text);}catch(error){console.debug?.('VN parse',error);}
  }
  ...
  async function update() {
    ...
    const rows=state.transcript||[];transcriptRows=rows;$('vnTranscript').innerHTML=rows.slice().reverse().map(row=>`<button class="vn-line ${Number(row.id)===lastLineId?'active':''}" data-vn-line="${Number(row.id)}">${esc(row.text)}</button>`).join('');
