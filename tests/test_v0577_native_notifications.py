...
from anime_mpv.notifications import maybe_handle_notification_helper, send_native_notification
...
def test_notification_uses_installed_app_bundle_helper(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("anime_mpv.notifications._notification_helper_path", lambda: helper)
    ...
    assert command == [
        str(helper),
        "--anime-mpv-native-notification",
        "Серия готова",
        "Название — серия 6",
    ]
    assert kwargs["env"]["ANIME_MPV_NOTIFICATION_HELPER_ACTIVE"] == "1"
...
def test_hidden_notification_mode_calls_native_framework(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "anime_mpv.notifications.send_native_notification_direct",
        lambda subtitle, message: calls.append((subtitle, message)) or True,
    )

    assert maybe_handle_notification_helper(
        ["--anime-mpv-native-notification", "Episode ready", "Example"]
    ) == 0
...
def test_failed_ready_notification_is_not_marked_delivered(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("anime_mpv.manager.send_native_notification", lambda *_args: False)
...
def test_test_notification_api_requests_permission_and_sends(tmp_path: Path, monkeypatch) -> None:
    ...
    cfg.library.database_path = tmp_path / "library.sqlite3"
    cfg.library.root_dir = tmp_path / "library"
    from anime_mpv.config import write_config
    ...
    monkeypatch.setattr(
        "anime_mpv.web_app.send_native_notification",
...
def test_installer_routes_helper_mode_through_app_bundle() -> None:
    ...
    assert "ANIME_MPV_NOTIFICATION_HELPER" in install
