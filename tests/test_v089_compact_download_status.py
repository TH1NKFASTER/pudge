...
def test_compact_download_status_shows_only_progress_and_eta() -> None:
    ...
    assert "const parts=[`${progress}%`]" in source
    assert "Downloading ${progress}%" not in source
    assert "Загрузка ${progress}%" not in source
    assert "ETA ${torrentEta(eta)}" in source
    assert "torrentRate(" not in source
    assert "download_speed" not in source
    assert "torrentTrafficActive&&Number.isFinite(eta)&&eta>0" in source
